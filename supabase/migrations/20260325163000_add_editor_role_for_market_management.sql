...
CREATE POLICY "Admins and editors can create markets"
ON public.markets
FOR INSERT
WITH CHECK (
...
CREATE POLICY "Admins and editors can update markets"
ON public.markets
FOR UPDATE
USING (
  public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'editor')
...
BEGIN
  ...
  IF NOT (
    ...
    public.has_role(v_user_id, 'editor')
  ) THEN
  ...
  IF p_winning_outcome THEN
    v_winning_pool := v_market.total_yes + v_market.seed_yes;
    v_losing_pool := v_market.total_no + v_market.seed_no;
  ELSE
    v_winning_pool := v_market.total_no + v_market.seed_no;
    v_losing_pool := v_market.total_yes + v_market.seed_yes;
  END IF;
